...
const NavItem = ({ item, level }) => {
    ...
    const itemIcon = item?.icon ? (
        ...
        <FiberManualRecordIcon
            sx={{
                width: appUI.isOpen.findIndex((id) => id === item?.id) > -1 ? 8 : 6,
                height: appUI.isOpen.findIndex((id) => id === item?.id) > -1 ? 8 : 6
            }}
    ...
    return (
        <ListItemButton
            ...
            selected={appUI.isOpen.findIndex((id) => id === item.id) > -1}
            onClick={() => itemHandler(item.id)}
        >
            <ListItemIcon sx={{ my: 'auto', minWidth: !item?.icon ? 18 : 36 }}>{itemIcon}</ListItemIcon>
            <ListItemText
                primary={
                    <Typography variant={appUI.isOpen.findIndex((id) => id === item.id) > -1 ? 'h5' : 'body1'}
                                color='inherit'>
                        {item.title}
                    </Typography>
                }
